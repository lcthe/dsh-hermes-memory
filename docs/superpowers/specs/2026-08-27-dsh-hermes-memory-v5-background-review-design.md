...
- 已加入有界 review output schema、本地二次校验和安全候选写入流水线；
- 已加入 `reviews` 表，保持 domain version 1 以兼容现有存储，并利用缺失表按空表加载的后端行为；
- 已加入默认关闭的后台复盘设置与客户端文案；
- 已接入 `session/flush`、可选 `ctx.jobs` 和可选结构化 `ctx.subagents`，无相关服务时安全跳过；
- 已完成 69 项测试、typecheck、build 和 npm pack 验证；
- 跨重启可靠队列、自动补偿和更丰富的 session 投影仍延期。
